...
public class GameSystem {
    ...
    static Occupation select(int i) throws OccupationOutOfRangeException {
        if (i == 0) {
            ...
            return new Worrior();//下一次攻击伤害必杀 战士
        } else if (i == 2) {
            return new Shooter();//对目标强制扣除两血 射手
        } else if (i == 3) {
            return new Wizard();//本回合后，受到一次必死伤害留1点血 法师
        } else if (i == 4) {
            return new Minister();//回复2点生命值，若本回合生命值下降且未死，回到6点生命值 牧师
        } else if (i == 5) {
            return new Mage();//交换与目标的hp 术士
        } else if (i == 6) {
            return new Priest();//掉2血加8mp 祭司
        } else if (i == 7) {
            return new Warlock();//若下两回合自己血量未变动，则目标被必杀 巫师
        } else if (i == 8) {
            return new Ranger();//目标防御之前不回mp 队员
        } else if (i == 9) {
            return new Shinobi();//此过后的第三回合对目标造成6点威力的攻击 忍者
        } else if (i == 10) {
            return new Killer();//发动一次威力等于回合数的攻击 杀手
        } else if (i == 11) {
            return new Paladin();//当回合以后免除一次对自己的stream伤害 骑士
        } else if (i == 12) {
            return new Sameral();//用1血分身抵挡本回合伤害 武士
        } else if (i == 13) {
            return new Assassin();//反弹当回合所有对自己释放的stream 刺客
        } else if (i == 14) {
            return new Agent();//强制结算 管理
        } else if (i == 15) {
            return new Knight();//3回合内mp无限 爵士
        } else if (i == 16) {
            return new Thief();//若本回合目标使用了技能，目标被必杀 小偷
        } else if (i == 36) {
            return new Summoner();//将target身上附着的buff换到自己身上，可指定此buff的target为哪一位角色 召唤
        } else if (i == 64) {
            return new Pirate();//随机给自己挂一个buff，对面目标可选 海盗
        } else {
